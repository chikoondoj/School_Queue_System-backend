...
class QueueController {
  ...
  async joinQueue(req, res) {
    try {
      ...
      const existingTicket = await queueService.getActiveTicketByStudent(
        studentId
      );
  ...
  async callNextStudent(req, res) {
    try {
      ...
      if (!result) {
        return res
          .status(404)
          .json({ message: "No students waiting in queue" });
      }
  ...
  async getServiceQueue(req, res) {
    try {
      ...
      const servingTicket = queueData.find(
        (ticket) => ticket.status === "serving"
      );
      const completedToday = queueData.filter(
        (ticket) =>
      ...
      const estimatedWaitTime =
        totalInQueue * serviceInfo[serviceId].estimatedTime;
      const averageServiceTime =
        completedToday.length > 0
          ? completedToday.reduce((sum, ticket) => {
              const serviceTime =
                new Date(ticket.updatedAt) - new Date(ticket.calledAt);
        ...
        data: {
          ...
          queue: {
              ...
              waitingTickets.length > 0
                ? {
                    ...
                    estimatedCallTime: new Date(
                      Date.now() + averageServiceTime * 60 * 1000
                    ),
                  }
                : null,
            waitingList: waitingTickets.map((ticket, index) => ({
              ...
              estimatedCallTime: new Date(
                Date.now() + (index + 1) * averageServiceTime * 60 * 1000
              ),
            })),
          },
          statistics: {
    ...
    try {
      ...
      if (ticket.status === "serving") {
        return res.status(400).json({
          success: false,
          message:
            "Cannot cancel - you are currently being served. Please complete your service or speak with staff.",
        });
      }
